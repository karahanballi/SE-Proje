...
// --- AYARLAR ---
const CEVAP_ANAHTARI: Record<string, string> = {
  "1": "A", "2": "A", "3": "B", "4": "C", "5": "D",
...
// --- TİPLER ---
type Option = "A" | "B" | "C" | "D";
type AnswersMap = Record<string, Option | null>;
type Point = { x: number; y: number };
type Corners = { tl: Point; tr: Point; bl: Point; br: Point };
type Score = { correct: number; wrong: number; empty: number };
...
function getProjectedPoint(corners: Corners, u: number, v: number): Point {
...
// Senin siyah kareleri arayan özel fonksiyon
function findSmartCorner(rgba: Uint8Array, w: number, h: number, corner: "tl" | "tr" | "bl" | "br"): Point | null {
  const W2 = Math.floor(w / 2);
  const H2 = Math.floor(h / 2);
  
  let startX = 0, endX = W2, startY = 0, endY = H2;
  // Hangi çeyrekte arayacağız?
  if (corner === "tr") { startX = W2; endX = w; }
  if (corner === "bl") { startY = H2; endY = h; }
  if (corner === "br") { startX = W2; endX = w; startY = H2; endY = h; }

  const step = 4;
  const threshold = 100; // Siyahlık limiti
  ...
  for (let y = startY; y < endY; y += step) {
    for (let x = startX; x < endX; x += step) {
      if (grayAt(rgba, w, h, x, y) < threshold) {
        // Siyah nokta bulduk! Bu bir kare mi?
        // Etrafındaki 10x10 alana bak, ne kadar siyah?
        let boxScore = 0;
        const boxSize = 10;
        ...
        // En yoğun siyah kutuyu hafızaya al
  ...
  // Eğer yeterince büyük bir kare bulamadıysa (maxScore düşükse) başarısız say
...
const DraggablePoint = ({ initialPos, onDrag, color = "#00ff00" }: { initialPos: Point, onDrag: (p: Point) => void, color?: string }) => {
  const pan = useRef(new Animated.ValueXY({ x: initialPos.x, y: initialPos.y })).current;

  // Pozisyon değiştiğinde güncelle (Props değişirse)
  useEffect(() => {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (e, gesture) => {
        ...
        onDrag({ 
            x: (pan.x as any)._offset + gesture.dx, 
            y: (pan.y as any)._offset + gesture.dy 
        });
      },
...
export default function OMR() {
  ...
  const [imageData, setImageData] = useState<{data: Uint8Array, w: number, h: number} | null>(null);
  ...
  const handleImage = async (uri: string) => {
    ...
    try {
      ...
      // 2. Piksel Verisini Al
      const bytes = b64ToU8(manip.base64!);
      const decoded = jpeg.decode(bytes, { useTArray: true });
      setImageData({ data: decoded.data as Uint8Array, w: decoded.width, h: decoded.height });

      // 3. SİYAH KARELERİ OTOMATİK ARA (Auto-Detect)
      const tl = findSmartCorner(decoded.data as Uint8Array, decoded.width, decoded.height, "tl");
      const tr = findSmartCorner(decoded.data as Uint8Array, decoded.width, decoded.height, "tr");
      const bl = findSmartCorner(decoded.data as Uint8Array, decoded.width, decoded.height, "bl");
      const br = findSmartCorner(decoded.data as Uint8Array, decoded.width, decoded.height, "br");

      if (tl && tr && bl && br) {
        // Bulduysa oraya yerleştir (Ekran koordinatına çevirerek)
        setCorners({
        ...
        // Bulamazsa varsayılan kenarlara koy
        const margin = 40;
        setCorners({
  ...
  const analyzeOMR = (rgba: Uint8Array, w: number, h: number, c: Corners, qCount: number) => {
    const answers: AnswersMap = {};
    const options: Option[] = ["A", "B", "C", "D"];
    
    // FORM GEOMETRİSİ (Senin resmine göre)
  ...
  const calculateScore = (userAns: AnswersMap) => {
    let c=0, w=0, e=0;
  ...
  if (step === "crop" && imageUri && viewDims && corners) {
    return (
        <View style={{flex:1, backgroundColor:'black'}}>
            <View style={{width: viewDims.w, height: viewDims.h, marginTop: 50, position:'relative'}}>
            ...
            <View style={styles.grid}>
                {Object.entries(answers).map(([q, ans]) => (
                    <View key={q} style={styles.row}>
                        ...
                        <Text style={{
                            // HATA ÇÖZÜMÜ: String(ans) kullanımı
